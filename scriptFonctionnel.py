...
# Function that reads the csv file generated by the script analyse.py
# to extract the gene ID and the product and attach it to the corresponding locus 
#
# Parameters: g (root graph),  geneID, geneProduct, Locus
# Return : None
#
def setNewProperties(g, geneID, geneProduct, Locus):
  with open('liste_locus.csv') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
      print(row)
      for n in g.getNodes():
        if(Locus[n] == row['Locus']):
          geneID.setNodeValue(n, row['GeneID'])
          geneProduct.setNodeValue(n, row['Product'])
